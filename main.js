...
const undatedListTop = el("undatedList"); // click-to-edit on undated chips
...
(() => {
  ...
  // NEW: keep reference to custom drag image so we can remove it
  let dragImgEl = null;
  ...
  // NEW: Create a small drag preview so it doesn't become a huge bar
  function setSmallDragImage(ev, chipEl){
    try{
      if (!ev.dataTransfer || !ev.dataTransfer.setDragImage) return;

      // Clean old one if any
      if (dragImgEl && dragImgEl.parentNode) dragImgEl.parentNode.removeChild(dragImgEl);
      dragImgEl = chipEl.cloneNode(true);

      // Make it compact regardless of original width:100%
      dragImgEl.style.width = "max-content";
      dragImgEl.style.maxWidth = "220px";
      dragImgEl.style.whiteSpace = "nowrap";
      dragImgEl.style.overflow = "hidden";
      dragImgEl.style.textOverflow = "ellipsis";
      dragImgEl.style.position = "fixed";
      dragImgEl.style.left = "-9999px";
      dragImgEl.style.top = "-9999px";
      dragImgEl.style.pointerEvents = "none";
      dragImgEl.style.transform = "none"; // avoid scaling from hover/drag CSS

      document.body.appendChild(dragImgEl);

      // Hotspot near left-middle
      ev.dataTransfer.setDragImage(dragImgEl, 12, 12);

      // Some browsers need the node to exist at least until dragstart finishes.
      // We'll remove it on dragend.
    } catch {
      // ignore: if a browser blocks setDragImage, we fall back to default
    }
  }
  ...
  function onDragStart(e){
    ...
    // NEW: custom drag image to avoid big bar
    setSmallDragImage(e, chip);
  }
  ...
  function onDragEnd(){
    ...
    // Remove custom drag image node
    if (dragImgEl && dragImgEl.parentNode){
      dragImgEl.parentNode.removeChild(dragImgEl);
    }
    dragImgEl = null;
